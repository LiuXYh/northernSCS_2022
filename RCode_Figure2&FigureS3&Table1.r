...
# Calculate the mean and standard deviation of BP, BA, and sBP, and Kruskal-Wallis test
Mean <- function(x) mean(x, na.rm = TRUE)
SD <- function(x) sd(x, na.rm = TRUE)
stat <- summaryBy(BP+BA+sBP~watermass, data = env, FUN = c(Mean, SD))
stat  # Table 1

kw <- kruskal(env$BP, env$watermass, p.adj = 'BH')
kw$groups
kw <- kruskal(env$BA, env$watermass, p.adj = 'BH')
kw$groups
kw <- kruskal(env$sBP, env$watermass, p.adj = 'BH')
kw$groups

# Calculate species diversity and phylogenetic diversity
...
# Calculate the mean and standard deviation of species diversity and phylogenetic diversity, and Kruskal-Wallis test
stat <- summaryBy(richness+shannon+mpd+SESmpd~watermass, data = env, FUN = c(Mean, SD))
stat  # Table 1

kw <- kruskal(env$richness, env$watermass, p.adj = 'BH')
kw$groups
kw <- kruskal(env$shannon, env$watermass, p.adj = 'BH')
kw$groups
kw <- kruskal(env$mpd, env$watermass, p.adj = 'BH')
kw$groups
kw <- kruskal(env$SESmpd, env$watermass, p.adj = 'BH')
kw$groups
...
geom_segment(data = db_rda_env.scaling2, aes(x = 0, y = 0, xend = dbRDA1*1.4, yend = dbRDA2*1.4), color = 'blue', size = 0.3) +
geom_text(data = db_rda_env.scaling2, aes(x = dbRDA1*1.4, y = dbRDA2*1.4, label = name), color = 'blue', size = 3) +
